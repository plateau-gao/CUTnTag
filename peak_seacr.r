...
for (type in peakType) {
    if (str_detect(type, "ctrl")) {
        ctrl_status = "ctrl"
    } else if (str_detect(type, "noctrl")) {
        ctrl_status = "noctrl"
    }
}
...
for (type in peakType) {
    for (id in sampleid) {
        overlap.gr = GRanges()
        for (rep in group) {
            peakInfo = read.table(str_subset(peakfile, paste0(rep, "_", id, "_seacr_", type)), header = FALSE, fill = TRUE)
...
write.csv(peak_summary[order(peakN$SampleId, peakN$Group, peakN$peakType), ], file = paste0("seacr_peak_summary.csv"), row.names = FALSE)
...
for (id in sampleid) {
    for (rep in group) {
        peakRes = read.table(str_subset(peakfile, paste0(rep, "_", id, "_seacr_", ctrl_status)), header = FALSE, fill = TRUE)
...
write.csv(frip_summary[order(frip_summary$SampleId, frip_summary$Group), ], file = paste0("seacr_frip_summary.csv"), row.names = FALSE) 
...
fig6D = frip %>% ggplot(aes(x = as.factor(SampleId), y = frip, fill = SampleId, label = round(frip, 2))) +
    ...
    xlab("")
png(file = paste0("seacr_frip_summary.png"), width = 1200, height = 900)
